...
public class Hbase {
    ...
    public static void main(String[] args) throws IOException {
    	...
    	init();
//        listTables();
    	long totalNum = 8000000000L;
    	int threadNum = 40;
    	int batchNum = 10000;
    	for (int i=0; i < threadNum; i++) {
    		MyThread th = new MyThread(connection, "usertable2", "usertable_", "family", "name", "value", 
            		threadNum, batchNum, totalNum, i);
    		th.start();
    	}
...
//    	scanData("usertable2", "", "");
    ...
    public static void init() {
         ...
         */
        configuration.set("hbase.zookeeper.property.clientPort", "2181");
        configuration.set("hbase.zookeeper.quorum", "192.168.0.230");
//        configuration.set("hbase.master", "192.168.0.230：16000");
...
//        }
    ...
    //批量插入
    public static void insterRows(String tableName, String rowkey, String colFamily, String col, String val, int threadNum, int batchNum, long totalNum, int targetId)
            throws IOException {
        Table table = connection.getTable(TableName.valueOf(tableName));
        long start = threadNum * targetId;
        long putNum = new Long(totalNum / batchNum).intValue() + 1;
        for(int j=0;j<putNum;j++) {
	        List<Put> puts = new ArrayList<Put>();
	        if(j + 1 == putNum)
	        {
	        	batchNum = new Long(totalNum - (batchNum * j)).intValue();
	        }
